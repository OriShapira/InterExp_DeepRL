...
class SummarizerClustering(SummarizerBase):
    ...
    def __init__(self, corpus, evaluateOnTheFly=False, filterSentsPerIteration=False):
        ...
        self.name = 'SummarizerClustering'
    ...
    def _getQuerySummaryText(self, query, numSentencesNeeded=-1, numTokensNeeded=-1):
        ...
        if query == '':
            finalSummaryTxtList, finalSummaryIds, numWordsInSummary = self._getNextGeneralSentences(numTokensNeeded)
            return finalSummaryTxtList, finalSummaryIds, numWordsInSummary
    ...
    #    return isPotentialSentence(sentence)
        ...
        if sentence.text[0] in ['"', "'", '`', '(', '[', '_', '-'] \
                or sentence.text[-1] != '.' \
                or sentence.lengthInWords < 6 \
                or sentence.lengthInWords > 35 \
                or sentence.lengthInChars < 20:
            return False
