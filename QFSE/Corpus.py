...
class Corpus():
    ...
    def getSentenceById(self, sentId):
        ...
        ## In an old version, there was a number on the doc id (e.g. 11_filename::15), so see if removing
        ## the leading number helps:
        #sentIdSplit = sentId.split('_', 1)
        ##if len(sentIdSplit) > 1 and sentIdSplit[0].isnumeric() and sentIdSplit[1] in self._sentIdToSentence:
        #if len(sentIdSplit) == 2 and sentIdSplit[1] in self._sentIdToSentence:
        #    return self._sentIdToSentence[sentIdSplit[1]]
        return None
