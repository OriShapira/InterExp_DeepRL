var requestUrl = "http://127.0.0.1:1389"; //"https://nlp.biu.ac.il/~oris/qfse_server/" //'https://u.cs.biu.ac.il/~shapiro1/qfse-api/ //'https://u.cs.biu.ac.il/summqa/';
var stopwatch = 0;
var m_topicId = '';
var algorithm = 'cluster'; // the default algorithm is the cluster algorithm
var algorithmModel = ''; // for rlmmr algorithm, this can be passed in through the URL, but if blank, the server will give a default model pair
...
function onInitFunc() {
    ...
    var algorithmModelVal = url.searchParams.get("algorithmModel");
    ...
    if (algorithmVal != null) {
        ...
        if (algorithmModelVal != null) {
            algorithmModel = algorithmModelVal;
        }
    }
...
function setAMTvalues(assignmentIdVal, hitIdVal, workerIdVal, turkSubmitToVal) {
    ...
    //// if atleast one of the AMT parameters were not given, just ignore all of them:
    //if (assignmentIdVal == null || hitIdVal == null || workerIdVal == null || turkSubmitToVal == null) {
    //    hitId = '';
    //    workerId = '';
    //    givenTurkSubmitTo = '';
    //    // in any case keep the assignment ID if this is an AMT preview session:
    //    if (assignmentIdVal != null) { //== 'ASSIGNMENT_ID_NOT_AVAILABLE') {
...
function setTopicsList(topicsList) {
    for (var topicInd = 0; topicInd < topicsList.length; topicInd++) {
        ...
        if (algorithmModel != '') {
            topicEle.href = topicEle.href+"&algorithmModel="+algorithmModel;
        }
...
function initializeTopic(topicId) { //, topicName) {
    if (topicId == null) {
        ...
        // if there is a keywords area, show it for the loading:
        ...
        sendRequest({"clientId": clientId, "request_get_initial_summary": {"topicId":topicId, "summaryType":summaryType, "algorithm": algorithm, "algorithmModel": algorithmModel, "summaryWordLength":initialSummWordLen, "questionnaireBatchIndex":questionnaireBatchInd, "timeAllowed": timeAllowed, "assignmentId": assignmentId, "hitId": hitId, "workerId": workerId, "turkSubmitTo": turkSubmitTo}});
        // summaryType, questionnaireBatchInd and timeAllowed are taken from the specific js file using functionaility here (e.g. functionailityQfse.js/functionailityIncrement.js)
        // the response will be sent to function setTopic asynchronously
    }
}
...
function setQueryResponse(queryResultInfo) {
    ...
    if (curLoadingIndicatorElement != null) {
        exploreList.removeChild(curLoadingIndicatorElement);//exploreList.lastChild);
...
function handleJsonReply(jsonObj) {
    ...
    else if ("reply_suggested_queries_update" in jsonObj) {
        setSuggestedQueries(jsonObj["reply_suggested_queries_update"]["keyPhraseList"]);
    }
    else {
        if (curLoadingIndicatorElement != null) {
            exploreList.removeChild(curLoadingIndicatorElement);//exploreList.lastChild);
...
function insertLoadingIndicatorInExplorationPane() {
    ...
    curLoadingIndicatorElement = listElement;
}
...
function stopExploringButtonOnClick() {
    /*retVal = practiceTaskMessage("Are you sure you acquired a sufficient amount of information?\nPress OK to move on, or Cancel to stay and keep exploring.", needWaitForRender=false, isOkCancel=true);
    if (retVal) {
        stopStopwatch();
...
function practiceTaskMessage(messageHtml, functionToExecute, isOkCancel=false) {
    ...
    var retVal = true;
    if (isPracticeTask) {
        messageStr = "Guiding instructions:\n\n" + messageStr;
        if (needWaitForRender) {
            // wait a bit for rendering and show the guidance message:
            setTimeout(function () {
                if (isOkCancel) { retVal = confirm(messageStr); } // true=OK, false=Cancel
                else { alert(messageStr); }
            }, 100);
        }
        else {
            if (isOkCancel) { retVal = confirm(messageStr); } // true=OK, false=Cancel
