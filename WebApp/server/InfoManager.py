...
class InfoManager:
    ...
    def initClientFunc(self, clientId, corpus, suggQuerGen, numSuggQueries, summarizer, topicId, questionnaireBatchIndex, timeAllowed, assignmentId,
        ...
        self.m_clientsInfo[clientId]['numSuggQueries'] = numSuggQueries
        ...
        self.m_clientsInfo[clientId]['suggestedQueries'] = suggQuerGen.getSuggestionsList(0)
    ...
    def getSummarizer(self, clientId):
        ...
        # take care of generating the summary.
        self.m_clientsInfo[clientId]['haveChanges'] = True
        return self.m_clientsInfo[clientId]['summarizer']

    def getSuggestedQueriesGenerator(self, clientId):
        # When the suggested queries generator is requested, it most likely means that a new list will be generated,
        # meaning there is a change in the data. We don't generate the queries here because it may take a long time and
        # hold up other requests. The web server handler is per request in different threads, so we let it
        # take care of generating the suggested queries.
        self.m_clientsInfo[clientId]['haveChanges'] = True
        return self.m_clientsInfo[clientId]['suggQuerGen']

    def getNumSuggestedQueriesNeeded(self, clientId):
        return self.m_clientsInfo[clientId]['numSuggQueries']
            ...
            for clientId in self.m_clientsInfo:
                try:
                    if self.clientHasChanges(clientId):# and self.clientIsReal(clientId):
                        ...
                        saveDict['summType'] = self.m_clientsInfo[clientId]['summarizer'].name #str(type(self.m_clientsInfo[clientId]['summarizer']))
                        saveDict['suggQType'] = self.m_clientsInfo[clientId]['suggQuerGen'].name
                        saveDict['summaries'] = self.m_clientsInfo[clientId]['summarizer'].getInfoForJson(
                            self.m_clientsInfo[clientId]['startTime'], suggestedQueriesObject=self.m_clientsInfo[clientId]['suggQuerGen'])
